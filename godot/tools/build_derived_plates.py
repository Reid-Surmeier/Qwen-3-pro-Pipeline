...
for hid in ["check-exp", "check-item"]:
    ...
    keep = ~(
        ((inner_px < 140).all(axis=-1))
        | ((inner_px[..., 2] > 90) & (inner_px[..., 2] > inner_px[..., 0] + 12))
    )
    interior_color = (
        np.median(inner_px[keep].reshape(-1, 3), axis=0) if keep.any() else [250, 250, 250]
    )
    crop[...] = panel
...
lines = [
    yy
    for yy in range(ty - 30, ty + th + 30)
    if ((ppx[yy, 12:518].astype(int) < 190).all(axis=-1)).mean() > 0.5
]
band_top = min(line for line in lines) + 2
band_bot = max(line for line in lines if line > band_top + 10)
