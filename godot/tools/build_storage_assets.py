...
def main() -> None:
    ...
    entries = [entry for entry in catalog["controls"] if entry.get("window") == "storage"]
    records: list[dict[str, object]] = []
    transparent = Image.new("RGBA", (1, 1), (0, 0, 0, 0))
    transparent_path = save(transparent, "transparent.png", records)
    transparent_variants = {phase: transparent_path for phase in ["idle", "hover", "pressed"]}
    ...
    for entry in grids:
        ...
        surfaces[slot] = {
            ...
            "state_set": {
                ...
                "modifier_selected": variants(crop, f"cell-{slot}-modifier", records, MODIFIER, 2),
                "transferring": variants(crop, f"cell-{slot}-transferring", records, MODIFIER, 3),
            },
        }

    collection = [f"stock-{index:03d}" for index in range(70)]
    labels = {item: f"Potion {index + 1:02d}" for index, item in enumerate(collection)}
    ...
    identity_surfaces = {
        item: occupied_slots[index % len(occupied_slots)] for index, item in enumerate(collection)
    }
    selection = control(
        "storage.items",
        "SelectionView",
        {"x": 88, "y": 30, "width": 420, "height": 315},
    ...
    for category, entry in zip(category_ids, category_entries, strict=True):
        ...
        category_surfaces[category] = {
            ...
            "state_set": {
                ...
                "selected": variants(crop, f"category-{category}-selected", records, BLUE, 2),
            },
        }
    categories = control(
        "storage.categories",
        "Tabs",
        {"x": 12, "y": 40, "width": 66, "height": 228},
        {"ready": transparent_variants},
        ["Activate"],
    ...
    close_entries = [
        entry for entry in entries if entry["type"] == "close" or entry.get("label") == "close"
    ]
    ...
    search = control(
        ...
        "TextField",
        field_geometry,
        {"empty": empty_field, "filtered": filtered_field},
        ["KeyCommand"],
    ...
    muted_up = variants(ImageEnhance.Brightness(arrow_up).enhance(0.58), "scroll-up-muted", records)
    muted_down = variants(
        ImageEnhance.Brightness(arrow_down).enhance(0.58), "scroll-down-muted", records
    )
    ...
    semantic_scroll = {state: scroll_transparent for state in ["at_start", "between", "at_end"]}
    scroll = control(
        "storage.scroll",
        "ScrollView",
        {"x": 508, "y": 30, "width": 14, "height": 315},
        semantic_scroll,
        ["Wheel", "Activate", "Drag"],
        [
        ...
        surfaces={
            "decrement": {
                "geometry": {"x": 0, "y": 0, "width": 14, "height": 16},
                "state_set": {"at_start": muted_up, "between": up_states, "at_end": up_states},
            },
            "track": {
                "geometry": {"x": 0, "y": 16, "width": 14, "height": 283},
                "asset": track_path,
            },
            "increment": {
                "geometry": {"x": 0, "y": 299, "width": 14, "height": 16},
                "state_set": {
                    "at_start": down_states,
                    "between": down_states,
                    "at_end": muted_down,
                },
            },
            "thumb": {
                "geometry": {"x": 0, "y": 16, "width": 14, "height": 56},
    ...
    ImageDraw.Draw(list_plate).rectangle(
        (88, 30, 507, 344), fill=(248, 248, 248, 255), outline=(170, 178, 192, 255)
    )
    ...
    storage_window = {
        ...
        "plates": {"expanded": expanded_path, "minimized": minimized_path, "list": list_path},
    ...
    manifest["windows"] = [item for item in manifest["windows"] if item.get("id") != "storage"]
    ...
    print(
        json.dumps(
            {
                "window": "storage",
                "controls": len(storage_window["controls"]),
                "slots": len(slots),
                "assets": len(records),
                "provider_requests": 0,
            }
