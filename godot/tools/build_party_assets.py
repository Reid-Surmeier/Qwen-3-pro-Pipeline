...
def phase_variants(
    image: Image.Image, stem: str, records: list[dict], disabled: bool = False
) -> dict:
    ...
    if disabled:
        ...
        return {
            ...
            for phase, value in (("idle", idle), ("hover", idle), ("pressed", pressed))
        }
    hover = idle.copy()
...
def choice_surface(
    base: Image.Image,
    selected_disc: Image.Image,
    unselected_disc: Image.Image,
    selected: bool,
    disc_position: tuple[int, int],
) -> Image.Image:
    result = base.copy()
    result.paste(
        selected_disc if selected else unselected_disc,
        disc_position,
        selected_disc if selected else unselected_disc,
    )
    return result
...
def main() -> None:
    ...
    ImageDraw.Draw(blank).rectangle(
        (0, 0, blank.width - 1, blank.height - 1), outline=(120, 120, 120, 255)
    )
    ...
    for index, (member_id, label, current, maximum) in enumerate(MEMBERS):
        ...
        unavailable = phase_variants(row, f"member-{member_id}-unavailable", records, disabled=True)
        ...
        display_facts.extend(
            [
                {
                    "id": f"member-{index + 1}-label",
                    "text": label,
                    "geometry": [34, 33 + index * 34, 170, 16],
                },
                {
                    "id": f"member-{index + 1}-hp",
    ...
    controls.append(
        control(
            ...
            ["Activate"],
            [{"gesture": "Activate", "action": "SelectPartyMember"}],
            ["unselected", "selected", "unavailable"],
            "unselected",
            value={
                "items": [entry[0] for entry in MEMBERS],
                "initial": MEMBERS[0][0],
                "details": details,
    ...
    for index, name in enumerate(ACTION_NAMES):
        ...
        disabled = phase_variants(crop, f"action-{name}-disabled", records, disabled=True)
        if name == "leave":
        ...
        controls.append(
            control(
                ...
                state_set,
                ["Activate"],
                [{"gesture": "Activate", "action": "ActivatePartyAction"}],
                states,
                initial,
                value={"action_id": f"party.action.{name}"},
            )
        )
    ...
    for choice, base, disc_at in (("friends", friends_base, (2, 1)), ("party", party_base, (2, 1))):
        choice_records[choice] = {}
        for semantic, selected in (("unselected", False), ("selected", True)):
            assembled = choice_surface(base, selected_disc, unselected_disc, selected, disc_at)
    ...
    controls.append(
        control(
            ...
            ["Activate"],
            [{"gesture": "Activate", "action": "SelectPartyMode"}],
            ["selected", "unselected"],
            "selected",
            value={"choices": ["friends", "party"], "initial": "party"},
            surfaces={
                "friends": {
                    "geometry": {"x": 0, "y": 0, "width": 71, "height": 23},
                    "state_set": choice_records["friends"],
                },
                "party": {
                    "geometry": {"x": 81, "y": 0, "width": 95, "height": 23},
        ...
        "state_adapter": {
            ...
            "actions": {
                "party.action.memo": {
                    ...
                    "reason": "Source icon behavior is unattested",
                },
                "party.action.info": {
                    "permission": "unavailable",
                    "reason": "Source icon behavior is unattested",
    ...
    manifest["windows"] = [window for window in manifest["windows"] if window.get("id") != "party"]
    ...
    (OUTPUT / "manifest.json").write_text(json.dumps(artifact, ensure_ascii=False, indent=2) + "\n")
