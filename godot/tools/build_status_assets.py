...
def main() -> None:
    ...
    for name, fact in ATTRIBUTES.items():
        ...
        available = name != "int"
        disabled = {"idle": transparent, "hover": arrow["hover"], "pressed": arrow["pressed"]}
        surface_states = {
            "available": arrow,
            "disabled": disabled,
        }
        controls.append(
            control(
                ...
                ["available", "disabled"],
                "available" if available else "disabled",
                value={
                    "minimum": int(fact["base"]),
                    "maximum": 99,
                    "current": int(fact["base"]),
                    "target": int(fact["base"]),
                    "step": 1,
                },
                surfaces={
                    "decrement": {
                        "geometry": {"x": 59, "y": 0, "width": 22, "height": 25},
                        "state_set": {
                            "available": transparent_states,
    ...
    manifest["windows"] = [
        entry for entry in manifest["windows"] if entry.get("id") != "status"
    ] + [window_spec]
