...
RECTS = json.loads(
    (ROOT.parent / "artifacts/references/ro-hud-fullscreen/window-rects.json").read_text()
)
...
results.append(
    {
        "window": "magenta-gutter",
        "rect": [gx, gy, gw, gh],
        "matching_fraction": round(frac, 4),
        "passed": frac >= 0.8,
    }
)
...
info = {
    "window": "game-backdrop (informational)",
    "changed_fraction": round(float((delta > CHANNEL_TOLERANCE).mean()), 5),
    "passed": True,
}
results.append(info)
