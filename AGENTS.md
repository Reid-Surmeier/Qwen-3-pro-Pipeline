...
- Issue-scoped paid verification may use OpenRouter only. The authoritative, human-approved Issue must name the question, model, initial batch, stopping condition, and any authorization for further output. Use the smallest useful batch and stop when the named question is answered; the absence of a repository-wide numeric cap does not authorize a broader batch. Do not use `provider: auto` or direct Alibaba under this allowance.
...
- Treat an ambiguous possibly billed request as spent until it is reconciled.
