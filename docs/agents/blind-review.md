...
- **Hermes on the host, sandbox in Docker.** The reviewer runs under a dedicated `blind-review` Hermes profile — created fresh, so no user memory, rules, or history reach its context; only the OpenRouter key is copied in. The orchestrator writes the sandbox settings (image, mounts, `docker_network: none`) into that profile's `config.yaml` each round, because Hermes treats config.yaml's `terminal` section as authoritative over shell env — the one channel that cannot be silently overridden back to a host shell. Every command, file read, and screenshot happens inside the container, and Hermes's vision resolver reads captured frames from inside it. After any Hermes update, re-verify with a cheap probe that the first terminal call lands in the sandbox image, not the host.
