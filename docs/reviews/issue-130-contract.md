...
2. Activate and DoubleActivate are real pointer paths. Activate selects one declared slot; Equipment DoubleActivate atomically unequips into the selected Inventory slot. Inventory DoubleActivate on the Equip tab resolves to the explicit `EquipInventoryItem` manifest action before local detail handling. Accepted transactions advance both public versions once.
...
4. Equip, unequip, and displacement commit both slot maps or neither. Invalid slots, empty sources, and stale versions return named errors and preserve both supplied maps byte-for-byte. A real invalid-destination drag includes before/mid/after frames; its committed before and after frames are exactly identical. The verifier independently recomputes before-to-mid metrics, requires positive transient change inside the intended region, and requires zero transient change inside the invariant region.
5. A committed displacement visibly updates both windows with source-owned imagery; factual QA state names the logical identity, actual TextureRect visibility, and actual loaded resource path. Missing foreign assets, undeclared identity surfaces, malformed empty-slot flags, and absent available State Sets fail construction validation.
...
8. The Equipment Items Play Log binds to candidate `d1385369e47754f7a75533319b1d165f7237d5ac`, exercises nine accepted/rejected paths with 32 hash-verified frames, and reports zero actionable console or page errors.
9. The native 198-contract Image 79 registry, 250 Python tests, 19 Node tests, compilation, and diff check pass on the exact candidate. Options, Skill Tree, Inventory, Storage, and Equipment Card browser Play Logs are rerun against the same SHA after shared SelectionView, ControlRuntime, ControlWindow, Desktop Action Router, and deterministic Play Log verifier changes.
