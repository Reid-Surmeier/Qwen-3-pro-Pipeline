...
4. Equip, unequip, and displacement commit both slot maps or neither. Invalid slots, empty sources, and stale versions return named errors and preserve both supplied maps byte-for-byte. A real invalid-destination drag includes before/mid/after frames; its committed before and after frames are exactly identical.
...
8. The Equipment Items Play Log binds to candidate `9dadca2845be0048b08ae9fe767afd16028cbd3d`, exercises nine accepted/rejected paths with 32 hash-verified frames, and reports zero actionable console or page errors.
9. The native 198-contract Image 79 registry, 249 Python tests, 19 Node tests, compilation, and diff check pass on the exact candidate. Options, Skill Tree, Inventory, Storage, and Equipment Card browser Play Logs are rerun against the same SHA after shared SelectionView, ControlRuntime, ControlWindow, Desktop Action Router, and deterministic Play Log verifier changes.
