...
if HAVE_PIL:
    from scripts import assemble_custom_filters_ro_v004 as assembly


@unittest.skipUnless(HAVE_PIL, "Pillow is not installed")
class CustomFiltersRoAssemblyV004Tests(unittest.TestCase):
    @classmethod
    ...
    def test_body_and_title_use_source_measured_depth_palettes(self) -> None:
        self.assertEqual(assembly.LABEL_INK, (46, 69, 96, 255))
        self.assertEqual(assembly.BODY_NEAR_DEPTH_INK, (177, 199, 228, 255))
        self.assertEqual(assembly.BODY_FAR_DEPTH_INK, (188, 197, 206, 255))
        self.assertEqual(
            assembly.LABEL_TEXT_EFFECTS,
            (
        ...
        self.assertEqual(assembly.TITLE_HIGHLIGHT_INK, (252, 244, 254, 255))
        self.assertEqual(assembly.TITLE_DEPTH_INK, (94, 101, 107, 255))
        self.assertEqual(
            assembly.TITLE_TEXT_EFFECTS,
            (
                ((1, 1), assembly.TITLE_DEPTH_INK),
                ((-1, -1), assembly.TITLE_HIGHLIGHT_INK),
                ((0, 0), assembly.TITLE_INK),
            ),
        )

        label_pixels = set(self.closed.crop((14, 30, 90, 44)).getdata())
        self.assertTrue(
        ...
        self.assertTrue(
            {assembly.TITLE_INK, assembly.TITLE_HIGHLIGHT_INK, assembly.TITLE_DEPTH_INK}
            <= title_pixels
        )

        source_body_pixels = set(self.source.crop((50, 105, 190, 270)).getdata())
        self.assertTrue(
            {
                assembly.LABEL_INK,
                assembly.BODY_NEAR_DEPTH_INK,
                assembly.BODY_FAR_DEPTH_INK,
            }
            <= source_body_pixels
        )
        with Image.open(assembly.TITLE_TEXT_AUTHORITY) as title_authority:
            authority_pixels = set(title_authority.convert("RGBA").getdata())
        self.assertTrue(
            {
                assembly.TITLE_INK,
                assembly.TITLE_HIGHLIGHT_INK,
                assembly.TITLE_DEPTH_INK,
            }
            <= authority_pixels
        )

    def test_title_treatment_authority_is_hash_locked(self) -> None:
        self.assertEqual(
            assembly.sha256(assembly.TITLE_TEXT_AUTHORITY),
            assembly.TITLE_TEXT_AUTHORITY_SHA256,
        )
        self.assertEqual(
            assembly.sha256(assembly.TITLE_FONT),
            assembly.TITLE_FONT_SHA256,
        )
    ...
    def test_popup_aligns_to_sort_field_and_every_option_is_contained(self) -> None:
        ...
        for index, option in enumerate(assembly.SORT_OPTIONS):
            ...
            with self.subTest(option=option):
                for (offset_x, offset_y), _fill in assembly.VALUE_TEXT_EFFECTS:
                    self.assertGreaterEqual(bounds[0] + offset_x, 1)
                    self.assertGreaterEqual(bounds[1] + offset_y, 1)
                    self.assertLessEqual(bounds[2] + offset_x, popup_width)
                    self.assertLessEqual(bounds[3] + offset_y, popup_height)
    ...
    def test_v001_v002_and_v003_are_retained_as_rejected_history(self) -> None:
        ...
        self.assertTrue(assembly.REJECTED_V003.exists())
