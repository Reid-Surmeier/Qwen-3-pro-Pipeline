...
"""Build Issue #138 v004 entirely on the source's native pixel grid.
...
nearest-neighbor enlargement. Body and title glyphs use source-measured native
offset layers instead of flat lettering or independently hinted 40px text.
"""
...
DEFAULT_OUTPUT = ROOT / "artifacts/runs/custom-filters-ro-assembly-v004"
...
TITLE_INK = (0, 1, 7, 255)
TITLE_HIGHLIGHT_INK = (252, 244, 254, 255)
TITLE_DEPTH_INK = (94, 101, 107, 255)
...
def make_contact_sheet(source: Image.Image, rejected: Image.Image, closed: Image.Image, open_state: Image.Image) -> Image.Image:
    ...
    draw.text((margin, 7), "style source / rejected v003", fill=(255, 255, 255))
    y = 28
    sheet.paste(source, (margin, y))
    sheet.paste(rejected, (source.width + 2 * margin, y))
    y += max(source.height, rejected.height)
    draw.text((margin, y + 9), "v004 closed - source-depth native text, exact edge caps and controls", fill=(255, 255, 255))
    y += 36
    sheet.paste(closed_review_image, (margin, y))
    y += closed_review_image.height
    draw.text((margin, y + 9), "v004 open - source-depth deterministic text over masked Qwen surface", fill=(255, 255, 255))
    y += 36
...
def main() -> int:
    ...
    verification = {
        "issue": 138,
        "assembly": "custom-filters-ro-v004",
        ...
        "rejected_parents": [repo_path(REJECTED_V001), repo_path(REJECTED_V002), repo_path(REJECTED_V003)],
    ...
    run = {
        ...
        "rejected": ["custom-filters-ro-assembly-v001", "custom-filters-ro-assembly-v002", "custom-filters-ro-assembly-v003"],
