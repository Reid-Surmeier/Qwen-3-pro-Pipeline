...
def text_width(font, s: str) -> int:
    if " " in s:
        words = s.split(" ")
        return (sum(int(font.getbbox(w)[2]) for w in words)
                + WORD_GAP * (len(words) - 1))
...
def build(spec: dict) -> Image.Image:
    ...
    match_w = (text_width(label, spec["match_label"]) + 8 + BW + 4
               + text_width(body, spec["any"]) + 16 + BW + 4
               + text_width(body, spec["all"]) + 12
    ...
    ref = Image.open("artifacts/references/museum-filter-retro-skin-v001/"
                     "style-ro-options-window-flat-rgb.png").convert("RGB")
    bar = ref.crop((600, 45, 1372, 124)).resize((interior_w, TITLE_H), Image.LANCZOS)
    img.paste(bar, (ox, oy))
    img.paste(bead, (ox + 3, oy + (TITLE_H - bead.height) // 2))
    draw_text(img, (ox + 3 + bead.width + 5, oy + (TITLE_H - 12) // 2 - 1),
              spec["title"], title_f, BODY_INK, highlight=WHITE)
    img.paste(close, (ox + interior_w - close.width - 3, oy + (TITLE_H - close.height) // 2))
    img.paste(bead, (ox + interior_w - close.width - 3 - bead.width - 5,
                     oy + (TITLE_H - bead.height) // 2))
    ...
    mx = x0 + lw + 8
    img.paste(box_on, (mx, y + 1)); mx += BW + 4
    mx += draw_text(img, (mx, y + 3), spec["any"], body, BODY_INK) + 16
    img.paste(box_off, (mx, y + 1)); mx += BW + 4
...
def main() -> int:
    ...
    print(f"{a.output} ({img.width * a.scale}x{img.height * a.scale}, native {img.width}x{img.height})")
    return 0
