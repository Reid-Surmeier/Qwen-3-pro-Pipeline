...
"""Build Issue #138 v003 entirely on the source's native pixel grid.
...
DEFAULT_OUTPUT = ROOT / "artifacts/runs/custom-filters-ro-assembly-v003"
...
BODY_TEXT_RULES = (
    {"text": "Custom filters:", "xy": (14, ROW_BASELINES[0]), "size": BODY_FONT_SIZE, "fill": LABEL_INK},
    {"text": "Images per page:", "xy": (13, ROW_BASELINES[1]), "size": BODY_FONT_SIZE, "fill": LABEL_INK},
    {"text": "20", "xy": (102, ROW_BASELINES[1]), "size": BODY_FONT_SIZE, "fill": BODY_INK},
    {"text": "Sort by:", "xy": (160, ROW_BASELINES[1]), "size": BODY_FONT_SIZE, "fill": LABEL_INK},
    {"text": "Relevance", "xy": (208, ROW_BASELINES[1]), "size": BODY_FONT_SIZE, "fill": BODY_INK},
    {"text": "On", "xy": (63, ROW_BASELINES[2]), "size": BODY_FONT_SIZE, "fill": BODY_INK},
    {"text": "Off", "xy": (100, ROW_BASELINES[2]), "size": BODY_FONT_SIZE, "fill": BODY_INK},
)
...
def assemble_closed(source: Image.Image) -> tuple[Image.Image, Image.Image]:
    ...
    draw_text(output, (15, 9), EXACT_COPY["title"], size=TITLE_FONT_SIZE, fill=BODY_INK, highlight=WHITE)
    ...
    for rule in BODY_TEXT_RULES:
        draw_text(output, rule["xy"], rule["text"], size=rule["size"], fill=rule["fill"])
...
def make_contact_sheet(source: Image.Image, rejected: Image.Image, closed: Image.Image, open_state: Image.Image) -> Image.Image:
    ...
    draw.text((margin, 7), "style source / rejected v002", fill=(255, 255, 255))
    y = 28
    sheet.paste(source, (margin, y))
    sheet.paste(rejected, (source.width + 2 * margin, y))
    y += max(source.height, rejected.height)
    draw.text((margin, y + 9), "v003 closed - native 10px text, exact edge caps and controls", fill=(255, 255, 255))
    y += 36
    sheet.paste(closed_review_image, (margin, y))
    y += closed_review_image.height
    draw.text((margin, y + 9), "v003 open - native deterministic text over masked Qwen surface", fill=(255, 255, 255))
    y += 36
...
def main() -> int:
    ...
    with Image.open(REJECTED_V002) as rejected:
    ...
    verification = {
        "issue": 138,
        "assembly": "custom-filters-ro-v003",
        ...
        "rejected_parents": [repo_path(REJECTED_V001), repo_path(REJECTED_V002)],
        "font": repo_path(FONT),
    ...
    run = {
        ...
        "rejected": ["custom-filters-ro-assembly-v001", "custom-filters-ro-assembly-v002"],
