"""Render the five DIS Data Issue pages through the live ComfyUI router.

Per page: upload donor+art references, submit the four-node graph
(LoadImage x2 -> ImageBatch -> QwenImage3Render -> SaveImage), poll history,
download outputs, and record workflow/prompt-id/response/hashes.
...
RUN = "/home/reidsurmeier/.qwen-dis-pages-wt/artifacts/runs/dis-data-issue-pages-v001"
PAGES = ["p1-cover", "p2-contents", "p3-paglen", "p4-losse", "p5-herndon"]
POLL_SECONDS = 5
TIMEOUT_SECONDS = 900
...
def main() -> None:
    ...
    for page in pages:
        ...
        donor = upload(f"{RUN}/inputs/{page}-donor.png", f"dis92-{page}-donor.png")
        art = upload(f"{RUN}/inputs/{page}-art.png", f"dis92-{page}-art.png")
        workflow = {
            ...
            "5": {
                ...
                "inputs": {"filename_prefix": f"dis92-{page}", "images": ["4", 0]},
            },
        }
