...
| Reject non-finite evidence | A JSON `NaN` worker safety factor raised an unhandled `ValueError` instead of producing machine-readable output | Every numeric memory and policy measurement must be finite; the CLI returns structured error JSON with status 2 |
